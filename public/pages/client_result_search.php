<?php 
 ...
 ?>
 	<title>UnitelRecru - Resultado da Pesquisa</title>
...
<body>
		...
		<section>
			<div class="container">
				<?php 
					...
					$sql = "select * from tb_anounce where direcion like '%$key_search%' or title like '%$key_search%' or subject like '%$key_search%'  or email like '%$key_search%';";
				...
				<div class="anounces" style="min-height: 600px;">
					...
					<div class="empresa">
	<?php
						$sql = "select * from tb_anounce where direcion='Empresa' and (title like '%$key_search%' or subject like '%$key_search%')  order by date_anounce desc limit 3";
							$result = $con->query($sql);


						if($result->num_rows>=1):
	?>
							<h1 class="main-title" style="border-bottom: none; font-size:1.1rem;  margin-top:.1rem">Últimos anúncios relativo a empresa</h1>
	...
	<?php
							$sql = "select * from tb_anounce where direcion='Candidato' and (title like '%$key_search%' or subject like '%$key_search%' or email like '%$key_search%')  order by date_anounce desc limit 3";
							$result = $con->query($sql);

						if($result->num_rows>=1):
	?>
							<h1 class="main-title" style="border-bottom: none; font-size:1.1rem;  margin-top:.1rem">Últimos anúncios relativo á canditura</h1>
	...
	?>
					...
					<?php endif;?>
							...
							<div class="candidatos">
	<?php
						$sql = "select * from tb_anounce where direcion='Mensagem' and (title like '%$key_search%' or subject like '%$key_search%' or email like '%$key_search%')  order by date_anounce desc limit 3";
							$result = $con->query($sql);

						if($result->num_rows>=1):
	?>
							<h1 class="main-title" style="border-bottom: none; font-size:1.1rem;  margin-top:.1rem">Últimos anúncios relativo a mensagem enviada</h1>
							<div class="anounces-candidato">
	<?php
							while($candidato = $result->fetch_assoc()):
	?>
